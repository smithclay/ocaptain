---
name: o-captain
description: Orchestrate tasks across multiple VMs using ocaptain fleet. Use when user wants to run commands, code changes, or any Linux task across multiple repos/VMs in parallel.
---

# O Captain - Fleet Task Orchestration
...
USAGE:
  ...
  ocaptain fleet status                      Show fleet status
  ocaptain fleet sink [--force] [--scuttle]  Destroy ships (--scuttle also destroys flagship)
  ocaptain ship create <owner/repo|name>     Create ship with repo, or empty VM if no "/"
  ocaptain ship destroy <id-prefix>          Destroy a ship (prefix matching)
  ocaptain ship ssh <id-prefix>              SSH into a ship (prefix matching)
  ocaptain inbox list [--status <s>]         List inbox messages (status: unread|done)
  ocaptain inbox send <recipient> <cmd>      Send command to recipient
  ocaptain inbox read <id>                   Read message and mark handled
...
- `ocaptain ship create smithclay/api` → creates `api-a1b2c3`
- `ocaptain ship create smithclay/api` → creates `api-x7y8z9` (new instance)
- `ocaptain ship create testvm` → creates `testvm-d4e5f6` (no repo)

Prefix matching works: `ocaptain ship ssh api-a1` matches `api-a1b2c3` (must be unique).
...
# Option 2: Add to .env file in ocaptain directory (persistent)
...
# Output:
# FLAGSHIP: flagship-ocaptain-abc123.vm.exe.dev
#
# SHIPS (2):
...
ocaptain inbox send captain@api-a1b2c3 "claude -p 'bump lodash to v5 in package.json and commit'"

# For shell commands
ocaptain inbox send captain@api-a1b2c3 "cd ~/api && npm test"

# Multiple commands
ocaptain inbox send captain@api-a1b2c3 "cd ~/api && npm install && npm test"
```
...
ocaptain inbox list --status done

# Read a specific message (marks as handled)
ocaptain inbox read <message-id>
```
...
```bash
ocaptain ship destroy api-a1b2c3
ocaptain ship destroy worker-x7   # prefix matching works
```
...
- Provide SSH command: `ocaptain ship ssh api-a1`

To destroy all ships at once:
```bash
ocaptain fleet sink          # Prompts for confirmation, destroys all ships
ocaptain fleet sink --force  # Skip confirmation
```

## Environment Requirements

- `GH_TOKEN` - GitHub PAT with repo access (required for `ship create` with repos)
  - **Best practice**: Store in `.env` file in ocaptain directory, or use `export GH_TOKEN="$(gh auth token)"`
  - **Never** pass on command line (visible in history/process list)
- Flagship must be initialized first via `ocaptain init`
...
> ocaptain ship create smithclay/worker
...
> ocaptain inbox send captain@api-a1b2c3 "claude -p 'run npm test and report results'"
Message sent: req-abc123 -> captain@api-a1b2c3

> ocaptain inbox send captain@worker-x7y8z9 "claude -p 'run npm test and report results'"
...
To inspect the failed ship:
  ocaptain ship ssh worker-x7

To scuttle when done:
  ocaptain ship destroy worker-x7
```
