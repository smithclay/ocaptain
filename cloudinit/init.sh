...
if [[ -n "$DOTFILES_PATH" ]]; then
  # Use local dotfiles (scp'd by ocaptain)
...
if [[ "${ROLE:-}" == "captain" && -n "${FLAGSHIP_SSH_DEST:-}" ]]; then
  ...
  sudo tee /etc/systemd/system/ocap-tunnel.service > /dev/null << TUNNEL_EOF
[Unit]
Description=ocaptain NATS tunnel to flagship
After=network.target
...
TUNNEL_EOF
  ...
  sudo systemctl daemon-reload
  sudo systemctl enable --now ocap-tunnel
fi

log "Setting up ship directories..."
mkdir -p ~/.local/bin ~/.ocaptain/ns/default
...
[Unit]
Description=ocaptain inbox scheduler
After=default.target

[Service]
ExecStart=%h/.local/bin/ocaptain _scheduler
Restart=always
RestartSec=5
